const options = {
  transitTimeType: {
    name: "tms_create_appointment_transittype",
    value: "exit",
  },
  accessPort: {
    name: "broker_create_appointment_arrivalport",
    value: 0,
  },
};
...
// select date
// document.querySelector("#finalSchedule > div > div > div.datepicker.col-md-6 > div.datepicker-days td:not(.disabled)").click();

// select calender
// const input = document.querySelectorAll("table > tbody > tr input[type=radio]").length > 3 ? document.querySelector("table > tbody > tr:has(input[type=radio]):nth-last-child(3) input[type=radio]") : document.querySelector("table > tbody > tr:has(input[type=radio]):nth-last-child input[type=radio]");
// input.checked = true;
